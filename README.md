...
```bash
npx pkg-urls-list ./project1 ./project2
```
...
```bash
npm install -g pkg-urls-list
```

Тогда можете использовать команду `pkg-urls-list` напрямую:
...
4. Сохранит результат в файл `pkg-urls-list.txt`
...
```bash
npx pkg-urls-list ./project1 ./project2 ./path/to/project3
```
...
4. Сохранит объединённый список в `pkg-urls-list.txt`
...
```
✅ Список зависимостей сохранён в файл: /path/to/pkg-urls-list.txt
```

Файл `pkg-urls-list.txt` будет содержать список URL-ов:
